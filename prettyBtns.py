...
def button(screen, screen2, position, text, size=50, color="yellow", bg="darkblue"):
...
def menu():
    ...
    quitBtn = button(screen, (400, 300), "Quit", size=40, color="yellow", bg="black")
    startBtn = button(screen, (500, 300), "Start", size=40)
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if quitBtn.collidepoint(pygame.mouse.get_pos()):
                    pygame.quit()
                elif startBtn.collidepoint(pygame.mouse.get_pos()):
                    start()
